/*2.2 Численно убедиться в справедливости равенства, для чего для
...
int main()
{
    double x = 0.0, elem, sum = 0.0, delta = 0.0;
    int i = 1;
    setlocale(0, "RUS");
    printf("Введите x и delta (через пробел): ");
    scanf_s("%lf %lf", &x, &delta);
    double si = sin(x), rs = 1 + delta;
    elem = x;
    for (i = 1; rs > delta; i++)
    {
        sum += elem;
